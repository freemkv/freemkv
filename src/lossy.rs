...
/// shell that platform compiles, and I narrowed the attribute by `cfg` twice
/// and got it wrong twice — first excluding macOS and Windows, which left it
/// dead on Linux, then including Windows, where the Windows CI compiler
/// promptly reported it dead anyway. A plain allow is the honest form: the
/// condition I was trying to express is "whichever platforms happen to route
/// through `summarize_stream`", which is not a property worth encoding in an
