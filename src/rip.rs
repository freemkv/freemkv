...
pub fn run(args: &[String]) {
    ...
    out.raw(
        Normal,
        &format!("freemkv rip v{}", env!("CARGO_PKG_VERSION")),
    );
    out.blank(Normal);
    ...
    out.raw_inline(
        Normal,
        &format!("{} ", strings::fmt("rip.opening", &[("device", &device)])),
    );
    ...
    if disc.encrypted {
        if let Some(ref aacs) = disc.aacs {
            ...
            out.raw(
                ...
                &format!(
                    "  {}:   {}",
                    strings::get("rip.verbose_aacs_version"),
                    aacs.version
                ),
            );
            out.raw(
                ...
                &format!(
                    "  {}:     {:?}",
                    strings::get("rip.verbose_key_source"),
                    aacs.key_source
                ),
            );
            out.raw(
                ...
                &format!(
                    "  {}:      {}",
                    strings::get("rip.verbose_disc_hash"),
                    aacs.disc_hash
                ),
            );
            if let Some(v) = aacs.mkb_version {
                out.raw(
                    Verbose,
                    &format!("  {}:    {}", strings::get("rip.verbose_mkb_version"), v),
                );
            }
            out.raw(
                Verbose,
                &format!(
                    "  {}: {}",
                    strings::get("rip.verbose_bus_encryption"),
                    aacs.bus_encryption
                ),
            );
            out.raw(
                ...
                &format!(
                    "  {}:      {}",
                    strings::get("rip.verbose_volume_id"),
                    hex(&aacs.volume_id)
                ),
            );
            out.raw(
                ...
                &format!(
                    "  {}:      {}",
                    strings::get("rip.verbose_unit_keys"),
                    aacs.unit_keys.len()
                ),
            );
            if let Some(ref rdk) = aacs.read_data_key {
                out.raw(
                    ...
                    &format!(
                        "  {}:  {:02x}{:02x}{:02x}{:02x}...",
                        strings::get("rip.verbose_read_data_key"),
                        rdk[0],
                        rdk[1],
                        rdk[2],
                        rdk[3]
            ...
            // handshake_error field removed from AacsState
        } else {
            out.print(Normal, "rip.aacs_no_keys");
            out.raw(
                Verbose,
                &format!("  ({})", strings::get("rip.verbose_handshake_hint")),
            );
        }
    }
    ...
    out.raw(
        Normal,
        &format!("{} ({}):", strings::get("rip.titles"), disc.titles.len()),
    );
    out.blank(Normal);
            ...
            for stream in &title.streams {
                match stream {
                    libfreemkv::Stream::Video(v) => {
                        out.raw(Normal, &format!("       {:?} {}", v.codec, v.resolution))
                    }
                    libfreemkv::Stream::Audio(a) => out.raw(
                        Normal,
                        &format!("       {:?} {} {}", a.codec, a.channels, a.language),
                    ),
                    libfreemkv::Stream::Subtitle(s) => {
                        out.raw(Normal, &format!("       {}", s.language))
                    }
                }
            }
        }
    ...
    if meta.extents.is_empty() {
        eprintln!("{}", strings::get("rip.no_extents"));
        std::process::exit(1);
    }
    ...
    if raw_mode {
        let raw =
            libfreemkv::M2tsStream::new(BufWriter::with_capacity(IO_BUF_SIZE, file)).meta(&meta);
...
impl<W: Write> Write for ProgressWriter<W> {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        if now.duration_since(self.last_update).as_secs_f64() >= 2.0 {
            ...
            let recent =
                (self.bytes_written - self.last_bytes) as f64 / (1024.0 * 1024.0) / interval;
            if recent > self.peak_speed {
                self.peak_speed = recent;
            }
            let pct = if self.total_bytes > 0 {
                (self.bytes_written as f64 / self.total_bytes as f64 * 100.0).min(100.0)
            } else {
                0.0
            };
