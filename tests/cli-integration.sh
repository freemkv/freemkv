...
# Opt-in, and OFF by default, for a reason worth stating rather than hiding.
#
# The ledger compares this run against a recorded baseline. That only means
# something if the INPUT is the same run to run — and here it is not: the media
# is regenerated by whichever ffmpeg the host has, and different ffmpeg builds
# encode differently. Per-platform keys were not enough; the macOS runner has
# the same platform string as the machine that seeded it and still produced
# different bytes, because it has a different ffmpeg.
#
# So in CI the ledger can only do one of two useless things: fail every run
# (noise), or record a fresh baseline every run and never compare (a green
# check that checks nothing). Both are worse than saying it did not run.
#
# The fix is a committed, deterministic media fixture instead of a generated
# one — then the baseline is reproducible everywhere and the ledger answers the
# question it was built for. Until that exists, this is a NAMED GAP.
#
# It stays fully working for local use, where the ffmpeg does not change
# between runs: FMKV_LEDGER=1 tests/cli-integration.sh
if [ -z "${FMKV_LEDGER:-}" ]; then
  skip "hash ledger — input media is ffmpeg-dependent, so a baseline is not reproducible across hosts (set FMKV_LEDGER=1 locally)"
elif [ -x "$LEDGER_SH" ] && command -v python3 >/dev/null 2>&1; then
